...
def get_outcome(user_wep, opponent_wep):
...
def get_round_score(user_wep, opponent_wep):
    ...
    Args:
        ...
        opponent_wep: string with opponent's chosen weapon
    Returns:
        int score for player based on their chosen weapon and outcome
        of round for them
    Raises:
        KeyError: provided user_wep param is not an available weapon
    """
    ...
    outcome = get_outcome(user_wep, opponent_wep)
...
def get_total_score(round_scores):
    ...
    Args:
        round_scores: iterable containing numeric values representing
        RPS round scores
    Returns:
...
with open("./day2/day2_input.txt") as rounds:
    ...
    while round != "":
        my_wep = get_weapon(round[-1])
        elf_wep = get_weapon(round[0])
        round_scores.append(get_round_score(my_wep, elf_wep))
        round = rounds.readline().strip()

# PART 1 Output
print(f"{get_total_score(round_scores)} is my total score")
